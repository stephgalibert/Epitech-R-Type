...
void SoundBox::addSample(std::string const& name, std::string const& path)
{
	SoundStruct *sound = NULL;
...
sf::Sound *SoundBox::getSound(std::string const& key) const
{
	if (_sample.find(key) == _sample.cend()) {
		throw (std::runtime_error("SoundBox: can not get " + key));
	}
	return (_sample.at(key)->sound);
}
